    ...
    { 
        ...
        foreach($dates as $d)
        {
            if(isset($data[$d]))
            {
                if(!is_array($data[$d]))
                {
                    throw new JapaModelException("'$d' action array instruction isnt an array"); 
                }
                else
                {
                    if(isset($data[$d][0]) && isset($data[$d][1]))
                    {
                        ...
                        {
                            throw new JapaModelException("Wrong '$d' array[0] value: ".$data[$d][0]); 
                        }
                        if(!preg_match("/^[0-9]{4}/",$data[$d][1]))
                        {
                            throw new JapaModelException("Wrong '$d' array[1] value: ".$data[$d][1]); 
                        }                    
                    }
                    else
                    {
                        throw new JapaModelException("Wrong (missing) '$d' array values");
                    }
                    if(isset($data[$d][2]) && isset($data[$d][3]))
                    {
                        if(!preg_match("/>|<|=|>=|<=|!=/",$data[$d][2]))
                        {
                            throw new JapaModelException("Wrong '$d' array[2] value: ".$data[$d][2]); 
                        }
                        if(!preg_match("/^[0-9]{4}/",$data[$d][3]))
                        {
                            throw new JapaModelException("Wrong '$d' array[3] value: ".$data[$d][3]); 
                        }                    
                    }             
                }
            }
